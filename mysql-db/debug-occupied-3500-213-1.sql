...
-- 3. Lockers with open transaction (PlaceID, Date_checkout null) but status null or MOD 10 = 0
...
JOIN fietsenstalling_sectie fs ON fs.sectieId = fp.sectie_id
WHERE fs.externalId = '3500_213_1'
  AND (fp.status IS NULL OR (fp.status % 10) = 0);
...
WHERE fs.externalId = '3500_213_1'
  AND ((fp.status IS NOT NULL AND (fp.status % 10) != 0) OR t.ID IS NOT NULL);
