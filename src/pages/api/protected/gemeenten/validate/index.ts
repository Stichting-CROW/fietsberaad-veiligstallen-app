...
export type GemeenteValidateResponse = {
  valid: boolean;
  message: string;
};
...
export default async function handle(
  ...
  const data = req.body as VSContactGemeente | undefined;
  if(!data) {
  ...
  switch (req.method) {
    case "POST": {
      ...
      if(isNew) {
        parseResult = gemeenteSchema.omit({ID: true}).safeParse(data); // will be set later
      } else {
        parseResult = gemeenteSchema.safeParse(data);
      }
      ...
      const uniqueFields: {field: keyof VSContactGemeente, message: string}[] = [
        {field: "CompanyName", message: "Er bestaat al een organisatie met deze naam"},
        {field: "ZipID", message: "Deze postcode bestaat al"},
