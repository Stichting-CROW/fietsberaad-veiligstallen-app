...
export default async function handle(
  ...
  try {
    ...
    for (let i = 0; i < sortedSiteIDs.length; i++) {
      ...
      // H1 for company with article count - add to PDF outline/bookmarks
      // Note: Company header will be on the first article's page
      const articleCount = companyArticles.length;
      const h1Text = `${companyName} (${articleCount} ${articleCount === 1 ? 'pagina' : 'pagina\'s'})`;
      
      // Add bookmark to PDF outline (this creates the navigable TOC in PDF viewers)
      // We'll add this bookmark when we create the first article of the company
      ...
      for (let j = 0; j < companyArticles.length; j++) {
        ...
        // Add company header and bookmark on first article of company
        if (j === 0) {
          const articleCount = companyArticles.length;
          const h1Text = `${companyName} (${articleCount} ${articleCount === 1 ? 'pagina' : 'pagina\'s'})`;
        ...
        // Note: Each article is on its own page, so no need to check for page breaks
      }
    }
