...
interface ExploreUserContactRoleComponentProps {
...
export const ExploreUserContactRoleComponent = ({
    selectedUserID,
    contacts,
    onShowAllContactsToggle,
    showAllContacts,
}: ExploreUserContactRoleComponentProps) => {
  ...
  useEffect(() => {
    const fetchUserContacts = async () => {
        const response = await fetch(`/api/protected/security_users/${selectedUserID}`);
    ...
    const renderSelectActiveOrganization = () => {
        ...
        if(userContacts && userContacts.length === 0) {
            console.warn("exploreusercontactrolecomponent - no user contacts found");
            return null;
        }
