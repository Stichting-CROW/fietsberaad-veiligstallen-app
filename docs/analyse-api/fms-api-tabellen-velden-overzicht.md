# FMS REST API v2/v3 – Overzicht gewijzigde tabellen en velden
...
Overzicht van alle `tabel.veld`-combinaties die door de API worden gewijzigd in **statische en eindtabellen**. Wachtrij-tabellen (`wachtrij_*`) en de cache-tabel `bezettingsdata_tmp` zijn weggelaten; die fungeren als tussenbuffer. Plek-gerelateerde velden (o.a. plekID, fietsenstalling_plek) staan in [Bijlage A: Fietskluizen](#bijlage-a-fietskluizen).

Gesorteerd op tabel. Kolommen per cluster: ● = veld wordt gewijzigd door operaties in dat cluster.
